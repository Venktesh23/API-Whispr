...
const calculateLocalScore = (specContent, endpoints) => {
  let score = 0
  const breakdown = {}
  ...
  const endpointsWithExamples = endpoints.filter(
    (ep) => ep.examples || (ep.responses && Object.values(ep.responses).some((r) => r.examples))
  ).length
  ...
  const usesProperMethods = endpoints.every((ep) =>
    ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS'].includes(ep.method)
  )
  const hasOperationIds = endpoints.filter((ep) => ep.operationId).length
  const designScore = Math.min(
    25,
  ...
  const endpointsWithResponses = endpoints.filter(
    (ep) => ep.responses && Object.keys(ep.responses).length > 0
  ).length
  ...
  const maintainabilityScore = (hasVersion ? 4 : 0) + (hasServers ? 4 : 0) + (hasContact ? 2 : 0)
  ...
  return {
    ...
    strengths:
      score >= 80
        ? ['Well-documented API', 'Good structure']
        : score >= 60
        ? ['Decent organization']
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (process.env.GEMINI_API_KEY) {
      try {
        const userPrompt = `Analyze this OpenAPI specification and calculate a health score:
...
${endpoints
  ...
  .join('\n')}`
        ...
        const result = await callGemini(HEALTH_SCORE_SYSTEM_PROMPT, userPrompt, {
          temperature: 0.3,
        ...
        console.log('Gemini analysis failed, using local calculation:', aiError.message)
      }
    }

    const result = calculateLocalScore(specContent, endpoints)
